jQuery.adminContract = {
		contractDataTable:null,
		initSearchDataTable : function() {
			if (this.contractDataTable == null) {
				this.contractDataTable = $('#dt_table_view').dataTable({
					...
					"aoColumnDefs" : [
						{
							...
							'fnRender' : function(oObj, sVal) {
								return"  <button class=\"btn2 btn-info\" onclick=\"$.adminContract.deleteContract("+oObj.aData.id+")\"><i class=\"icon-trash\"></i> 删除</button>" +
								" <button class=\"btn2 btn-info\" onclick=\"$.adminContract.showEdit("+oObj.aData.id+")\"><i class=\"icon-pencil\"></i>修改</button>";
							}
						},
					 {
						'aTargets' : [ '_all' ],
		...
		deleteContract :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/contract/delete/"+id,
	        			dataType : "json",
	        			success : function(json) {
	        				if(json.state=='success'){
	        					noty({"text":""+ json.resultMap.msg +"","layout":"top","type":"success","timeout":"2000"});
	        					$.adminContract.initSearchDataTable();
	        				}else{
		...
		showContractAddModal: function(id){
			$("#id").val(id);
			$('#_modal').modal({
			});
			$("#_modal").modal('show');
		...
		showEdit: function (id){
			...
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/contract/get/"+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.state=='success'){
